...
def MoveSShape(R1, R2, Y):
    ...
    if(V > servos.LINEAR_V_MAX):
        ...
        servos.setSpeedsVW(V*P1,2*math.pi*(1/(P1*Y))) #using w=2*pi*f as the formula to find w
        while(counts[0] <= 2*D1/servos.IN_PER_TICK or counts[1] <= 2*D1/servos.IN_PER_TICK): counts = encoders.getCounts()
        ...
        servos.setSpeedsVW(V*P2,-2*math.pi*(1/(P2*Y)))
        while(counts[0] <= 2*D2/servos.IN_PER_TICK or counts[1] <= 2*D2/servos.IN_PER_TICK): counts = encoders.getCounts()
